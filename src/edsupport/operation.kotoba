(ns edsupport.operation
  "OperationActor -- one educational-support operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (EdSupportOps-LLM) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Support Services Governor
  ...
  One graph run = one educational-support operation (intake -> advise
  ...
  decision to a human operator (provider staff). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:actuation/finalize-placement` ALWAYS reaches this node when the
  governor is clean -- see `edsupport.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [edsupport.edsupportadvisor :as edsupportadvisor]
...
(defn build
  ...
  `edsupport.store/Store`).
  opts:
    :advisor      -- a `edsupport.edsupportadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (edsupportadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; EdSupportOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (edsupportadvisor/-advise advisor store request)]
            {:proposal p :audit [(edsupportadvisor/trace request p)]})))

      ;; Support Services Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
