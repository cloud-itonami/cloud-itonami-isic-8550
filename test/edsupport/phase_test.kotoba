(ns edsupport.phase-test
  "The phase table as executable tests. The invariant this repo cannot
  regress on: `:actuation/finalize-placement` must NEVER be a member
  of any phase's `:auto` set."
  (:require [clojure.test :refer [deftest is testing]]
            [edsupport.phase :as phase]))

(deftest finalize-placement-never-auto-at-any-phase
  (testing "structural invariant: no phase, now or in the future entries, auto-commits a real placement finalization"
    (doseq [[n {:keys [auto]}] phase/phases]
      (is (not (contains? auto :actuation/finalize-placement))
          (str "phase " n " must not auto-commit :actuation/finalize-placement")))))
...
(deftest gate-escalates-a-clean-non-auto-write
  (is (= :escalate (:disposition (phase/gate 3 {:op :actuation/finalize-placement} :commit)))))
